...
clientui = uic.loadUiType("amend_tonghap.ui")[0]
...
class Main(QMainWindow, clientui):
    ...
    def qna_add(self):  # Q&A 질문하기
        if not self.text_s_qna_add.toPlainText():
            ...
            self.s_skt.send(f'^qnaqadd/{self.text_s_qna_add.toPlainText()}/^none'.encode())
            self.text_s_qna_add.clear()
    ...
    def info_start(self):  # 통계 보기 페이지 진입 함수
        ...
        self.ax.bar([1,2,3,4,5], [4,5,2,1,3],color=self.nationColor, alpha = 0.4)
        self.ax.set_ylabel("score", fontsize= 12)
        self.ax.set_xlabel("name", fontsize= 12)
        self.ax.set_title("Student score rate", fontsize= 15)
        self.canvas.draw()
    ...
    def info_show(self):
        ...
        #데이터 수신/발신
        self.ax.clear()
        self.ax.plot([1,2,3,4,5], [7,5,1,3,5], color ='red', marker='o')
        self.ax.set_xlabel("Exam_Question", fontsize= 12)
        self.ax.set_ylabel("number of incorrection", fontsize= 12)
        self.ax.set_title("Wrong answer rate", fontsize= 15)
        self.canvas.draw()
